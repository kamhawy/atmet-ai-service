...
public static class DatasetsEndpoints
{
    public static void MapEndpoints(RouteGroupBuilder group)
    {
        ...
        datasets.MapGet("/", ListDatasets)
            ...
            .WithDescription("""
                Returns the **latest version** per dataset name registered in the project.

                **Business use:** populate training job pickers or data governance dashboards.
                """)
        ...
        datasets.MapGet("/{name}/versions", ListDatasetVersions)
            ...
            .WithDescription("Returns **all versions** (immutable lineage) for a dataset logical name.")
            .Produces<List<DatasetResponse>>()
        ...
        datasets.MapGet("/{name}/versions/{version}", GetDataset)
            ...
            .WithDescription("Returns **metadata** for one `(name, version)` pair: storage hints, tags, connection, reference flags.")
